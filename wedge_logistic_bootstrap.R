...
# p(risk>low)
stem$risk1 <- ifelse(stem$risk=='L', 0, 1)

# p(risk>middle)
stem$risk2 <- ifelse(stem$risk=='H', 1, 0)
...
system.time({
  ...
  for(d in drug){
    ...
    for(i in 1:ncol(index.table)){
      ...
      # L vs M, H
      model1 <- glm(risk1~c3v1+c3v2+c3v3+c3v4+c3v6+c3v7+c3v8+c3v9+c3v10+c3v11+c3v12+c3v13+c3v14+c3v15+c3v16,
                    family = 'binomial', data = train.bootstrap)
      pred1 <- predict(model1, test, type = 'response')
      p.L <- 1 - pred1 
      stem[index.test, paste('p.L', i, sep = '_')] <- p.L
      
      # H vs L, M
      model2 <- glm(risk2~c3v1+c3v2+c3v3+c3v4+c3v6+c3v7+c3v8+c3v9+c3v10+c3v11+c3v12+c3v13+c3v14+c3v15+c3v16,
                    family = 'binomial', data = train.bootstrap)
...
saveRDS(stem, file = "result/logistic_wedge_bootstrap.RDS") 
...
dataset <- rep('wedge', 8000)
model <- rep('logistic', 8000)
...
figure.wedge.logistic <- data.frame(dataset, model, by, measurement, value); dim(figure.wedge.logistic)
saveRDS(figure.wedge.logistic, 'result/figure_wedge_logistic.RDS')
...
rate.drug$dataset <- 'wedge'
rate.drug$model <- "logistic"

saveRDS(rate.drug, file = "result/logistic_wedge_bootstrap_drug_rate.RDS") 
...
saveRDS(pred.drug, file = "result/logistic_wedge_bootstrap_drug_confusion.RDS") 

######################################################################
# analyze confusion matrix
######################################################################
pred.drug.logstic <- readRDS("result/logistic_wedge_bootstrap_drug_confusion.RDS")
pred.drug.randomforest <- readRDS("result/randomforest_wedge_bootstrap_drug_confusion.RDS")
pred.drug.merge <- merge(pred.drug.logstic, pred.drug.randomforest, by=c("Drug_Name","risk"))
