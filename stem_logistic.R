...
#drug.outlier <- c('Azimilide','Bepridil','Disopyramide','Domperidone',
#                  'Chlorpromazine','Clozapine','Ranolazine','Metoprolol')
...
for(d in drug){
  ...
  #model1 <- glm(risk1~arrhythmia, family = 'binomial', data = train)
  ...
  model2 <- glm(risk2~pred7+arrhythmia+fst_pro+maxpro+
                 foldaym+foldprolong+AMtwooutoffive,
               family = 'binomial', data = train)
  #model2 <- glm(risk2~arrhythmia, family = 'binomial', data = train)
...
####################################################################################
# concordance index
# run conda install -c conda-forge lifelines to install lifelines library in python
####################################################################################
