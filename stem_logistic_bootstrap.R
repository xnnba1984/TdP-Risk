...
system.time({
  ...
  for(d in drug){
    ...
    for(i in 1:ncol(index.table)){
      ...
      # L vs M, H
      model1 <- glm(risk1~pred7+arrhythmia+fst_pro+maxpro+
                      foldaym+foldprolong+AMtwooutoffive,
      ...
      # H vs L, M
      model2 <- glm(risk2~pred7+arrhythmia+fst_pro+maxpro+
                      foldaym+foldprolong+AMtwooutoffive,
                    family = 'binomial', data = train.bootstrap)
...
saveRDS(stem, file = "result/logistic_stem_bootstrap.RDS") 
stem <- readRDS("result/logistic_stem_bootstrap.RDS")
...
figure.stem.logistic <- data.frame(dataset, model, by, measurement, value); dim(figure.stem.logistic)
saveRDS(figure.stem.logistic, 'result/figure_stem_logistic.RDS')
...
saveRDS(rate.drug, file = "result/logistic_stem_bootstrap_drug_rate.RDS") 
...
saveRDS(pred.drug, file = "result/logistic_stem_bootstrap_drug_confusion.RDS") 

######################################################################
# analyze confusion matrix
######################################################################
pred.drug.logstic <- readRDS("result/logistic_stem_bootstrap_drug_confusion.RDS")
pred.drug.randomforest <- readRDS("result/randomforest_stem_bootstrap_drug_confusion.RDS")
